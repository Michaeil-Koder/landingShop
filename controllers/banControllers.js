...
exports.ban = async (req, res) => {
    try {
        ...
        } else if (user.updowngrade > finduser.updowngrade && user.role === "ADMIN" && finduser.role === "ADMIN") {
            return res.status(403).send({message: "شما مجاز به این درخواست نمی باشید"})
        } else if (user.role === "COLEADER" && finduser.role === "COLEADER") {
            return res.status(403).send({message: "شما مجاز به این درخواست نمی باشید"})
        } else if (user.role === "COLEADER" && finduser.role === "ADMIN") {
            return res.status(403).send({message: "شما مجاز به این درخواست نمی باشید"})
        }
        // timestamps persian
        ...
        const banRes = await banModel.create({
            user: finduser._id,
